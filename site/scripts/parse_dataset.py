...
def convert_text_fields_to_data(casted_dataset, manifest):
    ...
    for i,col in enumerate(transposed):
        if i == int(manifest['prediction_column']):
            ...
            if manifest["col_classes"][manifest["prediction_column"]] in ["Categorical", "Phrase", "Text"]:
                conversion_pipeline["label"] = list(set(labels))
                labels = [conversion_pipeline["label"].index(c) for c in labels]
            ...
            for term in unique_terms:
                ...
                internal_headers.append("Count of '"+str(term)+"' in "+str(manifest['headers'][i]))
            ...
            internal_headers.append(str(manifest['headers'][i]))
        else:
            ...
            if dist > 0:
                ...
                internal_headers.append(str(manifest['headers'][i])+" rescaled")
            if stdev > 0:
                detexted.append(((np.array(replaced)-average)/(stdev)).tolist())
                internal_headers.append(str(manifest['headers'][i])+" standardized")
            detexted.append(np.abs(replaced))
            internal_headers.append(str(manifest['headers'][i])+" absolute value")
    cleared_labels = []
    cleared_dataset = []
    ...
    conversion_pipeline['internal_headers'] = internal_headers
