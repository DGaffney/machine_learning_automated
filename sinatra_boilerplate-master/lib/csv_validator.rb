...
class CSVValidator
  ...
  def initialize(csv_data, filename, filesize_mb)
    return "Must be < 60mb in order to be analyzed - larger files coming in the future" if filesize_mb > 60
    self.filesize_mb = filesize_mb
    self.csv_data = csv_data #csv_data.shuffle.first(1000) if it needs to be shortened during validation
    transposable = self.csv_data.transpose rescue nil
    ...
    self.headers = header ? cols.collect(&:first) : 1.upto(cols.count).collect{|x| "Row #{x} (#{self.col_classes[x-1]})"}
  ...
  def get_classes(classes, header)
    ...
    classes.collect(&:uniq).each_with_index do |class_set, i|
      if class_set.include?(String)
        if (Math.log(self.csv_data[i].length, 10) - Math.log(self.csv_data[i].uniq.length, 10)) > 2
